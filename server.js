...
function load_data() {
    ...
    Message.deleteMany({}, function (err, result) {
        ...
        if (err) {
            ...
            fs.readdir("./gm_messages", function (err, files) {
                if (err) { return console.log('Unable to scan directory: ' + err); }
                files.forEach(function (file) {
                    const messageData = {
                        message_text: fs.readFileSync("./gm_messages/" + file).toString(),
            ...
            fs.readdir("./messages", function (err, files) {
                ...
                files.forEach(function (file) {
                    const messageData = {
                        message_text: fs.readFileSync("./messages/" + file).toString(),
    ...
    Image.deleteMany({}, function (err, result) {
        ...
        if (err) {
            ...
            fs.readdir("./images_compressed", function (err, files) {
                if (err) {
                    return console.log('Unable to scan directory: ' + err);
                }
                files.forEach(function (file) {
                    const imageData = {
                        image_content: fs.readFileSync("./images_compressed/" + file),
                    }
                    const image = new Image(imageData);
